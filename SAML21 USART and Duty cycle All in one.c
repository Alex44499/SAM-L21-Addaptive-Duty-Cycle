...
uint8_t p = 190; // Change initial point (Duty cycle)
...
void check_for_commands() {
	if (g_data_valid) {
		...
		if (input_buffer[0] == 'u') {
			...
			pwm_period += 10;
			p += 10; /////////////// new part it follows frequency
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		tcc0_cfg.pins.wave_out_pin[2]        = PIN_PA08E_TCC0_WO0;
		tcc0_cfg.pins.wave_out_pin_mux[2]    = MUX_PA08E_TCC0_WO0;
		
		tcc0_cfg.compare.match[3] = pwm_period / 2;
		tcc0_cfg.pins.enable_wave_out_pin[3] = true;
		tcc0_cfg.pins.wave_out_pin[3]        = PIN_PA09E_TCC0_WO1;
		tcc0_cfg.pins.wave_out_pin_mux[3]    = MUX_PA09E_TCC0_WO1;
		...
		tcc1_cfg.pins.wave_out_pin[2]        = PIN_PA10E_TCC1_WO0;
		tcc1_cfg.pins.wave_out_pin_mux[2]    = MUX_PA10E_TCC1_WO0;
						
		tcc1_cfg.compare.match[3] = pwm_period / 2;
		tcc1_cfg.pins.enable_wave_out_pin[3] = true;
		tcc1_cfg.pins.wave_out_pin[3]        = PIN_PA11E_TCC1_WO1;
		tcc1_cfg.pins.wave_out_pin_mux[3]    = MUX_PA11E_TCC1_WO1;
		...
		while (1) {
			...
			if (seconds_from_start < 2800 ){															// Run tcc0
			
			tcc0_instance.hw->CTRLA.reg |= TCC_CTRLA_ENABLE;
			duty_cycle = (measured_voltage1 / (measured_voltage1 + measured_voltage2)) * p;
			}
			
			else if (seconds_from_start == 2800 ){												// Stop tcc0
			tcc0_instance.hw->CTRLA.reg &= ~TCC_CTRLA_ENABLE;
			}
			
			else if ((seconds_from_start > 2800 ) && (seconds_from_start < 10000)){			// Run tcc1
			...
			//if (seconds_from_start < 3000 ){															// Run tcc0
				
				//tcc0_instance.hw->CTRLA.reg |= TCC_CTRLA_ENABLE;	
				//duty_cycle = (measured_voltage1 / (measured_voltage1 + measured_voltage2)) * 195;
				//}
				
				//else if (seconds_from_start == 2500 ){												// Stop tcc0
					//tcc0_instance.hw->CTRLA.reg &= ~TCC_CTRLA_ENABLE;
					//}
					
					//else if ((seconds_from_start > 2500 ) && (seconds_from_start < 10000)){			// Run tcc1
 						
						//tcc1_instance.hw->CTRLA.reg |= TCC_CTRLA_ENABLE;
 						//duty_cycle = (measured_voltage2 / (measured_voltage1 + measured_voltage2)) * 195;
 						//}
 						
// 						else if (seconds_from_start == 1000 ){										// Stop tcc1
// 							tcc1_instance.hw->CTRLA.reg &= ~TCC_CTRLA_ENABLE;
// 							}
// 							
// 							else if ((seconds_from_start > 1000 )&&(seconds_from_start < 1400 )){  
// 							
// 								tcc0_instance.hw->CTRLA.reg |= TCC_CTRLA_ENABLE;					// Run tcc0
// 								duty_cycle = (measured_voltage1 / (measured_voltage1 + measured_voltage2)) * 195;
// 								}
// 						
// 								else if (seconds_from_start == 1400 ){
// 									tcc0_instance.hw->CTRLA.reg &= ~TCC_CTRLA_ENABLE;				//Stop tcc0
// 									}
// 									
// 									else if ((seconds_from_start > 1400 ) && (seconds_from_start > 1800)){
// 										tcc1_instance.hw->CTRLA.reg |= TCC_CTRLA_ENABLE;				//Runtcc1
// 										duty_cycle = (measured_voltage2 / (measured_voltage1 + measured_voltage2)) * 195;
// 										}
// 										
// 										else if (seconds_from_start == 1800 ){										// Stop tcc1
// 											tcc1_instance.hw->CTRLA.reg &= ~TCC_CTRLA_ENABLE;
// 											}
// 											
// 											else if ((seconds_from_start > 1800 )&&(seconds_from_start < 2400 )){
// 												tcc0_instance.hw->CTRLA.reg |= TCC_CTRLA_ENABLE;					// Run tcc0
// 												duty_cycle = (measured_voltage1 / (measured_voltage1 + measured_voltage2)) * 195;
// 												}
// 												
